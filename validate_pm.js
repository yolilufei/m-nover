 ...
 */
const detectedWhichPMInuse = (pkgInfo) => {
  let pkgManager = "";
  if (pkgInfo.packageManager) {
    pkgManager = [PKG_MANAGER_NPM, PKG_MANAGER_YARN, PKG_MANAGER_PNPM].find(
      (pm) => pkgInfo.packageManager.startsWith(pm)
    );
  }
  if (existLockFile("package-lock.json")) {
    pkgManager = PKG_MANAGER_NPM;
  }
  if (existLockFile("yarn.lock")) {
    pkgManager = PKG_MANAGER_YARN;
  }
  if (existLockFile("pnpm-lock.yaml")) {
    pkgManager = PKG_MANAGER_PNPM;
  }
  const current_exec_pm = process.env.npm_execpath;
  if (pkgManager && current_exec_pm.indexOf(pkgManager) < 0) {
